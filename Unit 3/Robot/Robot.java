...
public class Robot extends Panel implements MouseListener, ActionListener, WindowListener
{
	...
	public void turn( int dir )
	{
		...
		switch (dir )
		{
			...
			case DIR_BACK 	:	direction = ( direction + 2 ) % 4 ;
								break ;
	...
	public void move( int dir )
	{
		...
		switch( dir )
		{
			...
			case 	DIR_LEFT:	{
									if	( check( DIR_LEFT ) == CELL_WALL || check( DIR_LEFT ) == CELL_OBSTACLE )
									{
										stage = FATAL_ERROR ;
									...
									switch ( direction )
									{
										case DIR_NORTH	: 	robot.x-- ;	break ;

										case DIR_SOUTH	: 	robot.x++ ;	break ;

										case DIR_EAST	: 	robot.y-- ;	break ;

										case DIR_WEST	: 	robot.y++ ;	break ;
									}
								}
								break ;

			case 	DIR_RIGHT:	{
									if	( check( DIR_RIGHT ) == CELL_WALL || check( DIR_RIGHT ) == CELL_OBSTACLE )
									{
										stage = FATAL_ERROR ;
